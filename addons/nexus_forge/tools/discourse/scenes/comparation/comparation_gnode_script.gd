...
func generate_node_dictionary() -> Dictionary:
	var comp_dictionary: Dictionary = NFDiscourseTool.get_comparation_structure()
	...
	comp_dictionary["operator"] = get_selected_operator()
...
func get_selected_operator() -> int:
	match operation_option.selected:
		0:
			return OP_EQUAL
		1:
			return OP_NOT_EQUAL
