...
func add_file(file_path: String) -> TreeItem:
	var new_item = create_item(file_root)
	new_item.set_text(0, file_path.get_file())
	return new_item


func has_file(file_path: String) -> bool:
	for child in file_root.get_children():
		if Strings.nocasecmp_equal(child.get_metadata(0)["path"], file_path):
			return true
	return false
