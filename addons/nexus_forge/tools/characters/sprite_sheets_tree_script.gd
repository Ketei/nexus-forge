...
func create_sheet_path(sprite_id: String, sprite_path: String) -> String:
	var new_sheet: TreeItem = create_item(root_tree)
	
	new_sheet.set_cell_mode(0, TreeItem.CELL_MODE_STRING)
	new_sheet.set_cell_mode(1, TreeItem.CELL_MODE_STRING)
	
	new_sheet.set_text(0, validate_sprite_id(sprite_id))
	new_sheet.set_metadata(0, new_sheet.get_text(0))
	new_sheet.set_text(1, sprite_path)
...
func on_item_edited() -> void:
	...
	if edited_column == 0:
		if edited_item.get_text(0) != edited_item.get_metadata(0):
			var from_name: String = edited_item.get_metadata(0)
			var new_name: String = validate_sprite_id(edited_item.get_text(0))
			if edited_item.get_text(0) != new_name:
				edited_item.set_text(0, new_name)
			edited_item.set_metadata(0, new_name)
			id_edited.emit(from_name, new_name)


func validate_sprite_id(variant_id: String) -> String:
	var desired_id: String = "new_sprite_sheet" if variant_id.is_empty() else variant_id
...
func get_sprites_data() -> Dictionary:
	...
	for alt in root_tree.get_children():
		data[alt.get_text(0)] = alt.get_text(1)
	return data


func get_variant_ids() -> Array:
	var variants: Array = []
	for variant in root_tree.get_children():
		variants.append(variant.get_text(0))
	return variants
