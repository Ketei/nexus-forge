@tool
extends Tree


const PERK_MAX_LEVEL: int = 9999
var root_tree: TreeItem


func _ready() -> void:
	root_tree = create_item()
	
	set_column_title(0, "ID")
	set_column_title(1, "Name")
	set_column_title(2, "Level")
	
	set_column_expand(0, true)
	set_column_expand(1, true)
	set_column_expand(2, false)
	
	set_column_custom_minimum_width(2, 80)
	
	#item_edited.connect(on_item_edited)


func add_perk(perk_id: String, perk_name: String, perk_level: int) -> void:
	var new_perk: TreeItem = create_item(root_tree)
	
	new_perk.set_cell_mode(0, TreeItem.CELL_MODE_STRING)
	new_perk.set_cell_mode(1, TreeItem.CELL_MODE_STRING)
	new_perk.set_cell_mode(2, TreeItem.CELL_MODE_RANGE)
	
	new_perk.set_editable(0, false)
	new_perk.set_editable(1, false)
	new_perk.set_editable(2, true)
	
	new_perk.set_selectable(0, false)
	new_perk.set_selectable(1, false)
	new_perk.set_selectable(2, true)
	
	new_perk.set_text(0, perk_id)
	new_perk.set_text(1, perk_name)
	new_perk.set_range_config(2, 0, perk_level, 1.0)


#func on_item_edited() -> void:
	#var perk: TreeItem = get_edited()
	#match get_edited_column():
		#0:
			#if perk.is_checked(0):
				#if perk.get_range(2) == 0:
					#perk.set_range(2, 1)
			#else:
				#if 0 < perk.get_range(2):
					#perk.set_range(2, 0)
		#2:
			#if perk.get_range(2) != 0:
				#if not perk.is_checked(0):
					#perk.set_checked(0, true)
			#else:
				#if perk.is_checked(0):
					#perk.set_checked(0, false)


func set_perk(perk_id: String, perk_level: int) -> void:
	for perk in root_tree.get_children():
		if perk.get_text(0) == perk_id:
			#perk.set_checked(0, perk_checked)
			perk.set_range(2, clampi(perk_level, 0, PERK_MAX_LEVEL))
			break


func get_selected_perks() -> Dictionary:
	var perks: Dictionary = {}
	
	for perk in root_tree.get_children():
		if 0 < perk.get_range(1):
			perks[perk.get_text(0)] = {"level": perk.get_range(1)}
	return perks


func clear_checks() -> void:
	for perk in root_tree.get_children():
		if perk.is_checked(0):
			perk.set_checked(0, false)


func clear_perks() -> void:
	for perk in root_tree.get_children():
		perk.free()
	
