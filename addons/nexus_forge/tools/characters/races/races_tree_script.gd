...
func _ready() -> void:
	...
	set_column_title(0, "Race ID")
	set_column_title(1, "Race Name")
...
func create_race(race_id: String, race_name: String) -> void:
	var new_race = create_item(root_tree)
	var valid_id: String = validate_id(race_id)
	var new_name = "New Race" if race_name.is_empty() else race_name
	
	new_race.set_editable(0, true)
	new_race.set_editable(1, true)
	
	new_race.set_text(0, valid_id)
	new_race.set_metadata(0, valid_id)
	
	new_race.set_text(1, new_name)
...
func on_item_selected() -> void:
	race_selected.emit(get_selected().get_text(0))


func validate_id(desired_id: String) -> String:
	var ideal_id: String = "new_race" if desired_id.is_empty() else desired_id
