...
@onready var call_opt_btn: OptionButton = $CallOptBtn


func _ready() -> void:
	graph_type = GraphType.CALL
	register_output_connection("call", 0, true)
	
	var idx: int = -1
	
	for call_id in NexusForge.Callables.get_callable_ids():
		idx += 1
		call_opt_btn.add_item(
				NexusForge.Callables.get_callable_name(call_id))
...
func _is_orphan() -> bool:
	if has_any_output_connection("call"):
		return get_output_connections("call")[0]._is_orphan()
	return true
...
func on_call_idx_selected(idx: int) -> void:
	...
	for arg_type in NexusForge.Callables.get_callable_args(call_opt_btn.get_item_metadata(idx)):
