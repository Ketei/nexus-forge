...
func _ready() -> void:
	...
	if not res_path.is_empty() and ResourceLoader.exists(res_path):
		var res_pre: Resource = load(res_path)
		if res_pre is NFCharacterDBRes:
	...
	if load_resources():
		...
		no_db_container = preload("res://addons/nexus_forge/scenes/characters/kinds_no_db.tscn").instantiate()
...
func on_create_new_resource() -> void:
	...
	resource_loader.file_mode = EditorFileDialog.FILE_MODE_SAVE_FILE
...
func on_open_char_resource() -> void:
	var resource_loader := preload("res://addons/nexus_forge/classes/resource_file_dialog.gd").new()
	resource_loader.file_mode = EditorFileDialog.FILE_MODE_OPEN_FILE
