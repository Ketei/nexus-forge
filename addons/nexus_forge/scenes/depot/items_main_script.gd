...
func _ready() -> void:
	var resource_path: String = ProjectSettings.get_setting(NFItemsRes.SETTINGS_PATH, "")
	
	if not resource_path.is_empty() and ResourceLoader.exists(resource_path):
		var pre_res: Resource = load(resource_path)
		if pre_res is NFItemsRes:
			items_resource = pre_res
...
func _on_create_resource_pressed() -> void:
	...
	if result[0]:
		...
		ProjectSettings.set_setting(NFItemsRes.SETTINGS_PATH, result[1])
...
func _on_load_resource_pressed() -> void:
	...
	if result[0]:
		...
		if res_pre != null and res_pre is NFItemsRes:
