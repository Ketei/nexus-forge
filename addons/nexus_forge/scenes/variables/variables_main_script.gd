...
func on_create_resource_pressed() -> void:
	...
	if result[0]:
		...
		ProjectSettings.set_setting(NFVariablesRes.SETTINGS_PATH, result[1])
...
func on_load_resource_pressed() -> void:
	var new_dialog := ResourceFileDialog.new()
	new_dialog.file_mode = EditorFileDialog.FILE_MODE_OPEN_FILE
	add_child(new_dialog)
	new_dialog.show()
...
func _on_folder_selected(path_to_folder: String) -> void:
	var variables: Dictionary = _variables_resource.get_variables(path_to_folder)
